...
// Clocks for controlling color cycling
uint16_t clk = 0;
uint16_t colclk = 0;

// Color masks
const uint16_t colormask[6] = {
  0b0000000000011111, // Blue
  0b0000001111111111, // Blue + Green
  0b0000001111100000, // Green
  0b1111101111100000, // Red + Green
  0b1111100000000000, // Red
  0b1111100000011111, // Red + Blue
};

const uint16_t levelmask[4] = {
  0b1111101111111111, // Brightest
  0b1001001001010010, // Medium
  0b0011100011100111, // Low
  0b0001100001100011, // Dimmest
};

// Look-up table we use to control color changing.
// Initialize with starting color (Blue)
uint16_t dotcolor[5] = {
  0b0000000000011111, // Brighest
  0b0000000000010010, // Med   
  0b0000000000000111, // Low
  0b0000000000000011, // Dimmest
  0b0000000000000000, // Black (off)
};
...
void loop(void) {
  ...
  for(j=63;j>0;j--) {
    for(i=0;i<32;i++) {
      bufcopy[i][j] = bufcopy[i][j-1];
      matrix.drawPixel(i,j,bufcopy[i][j]);
    }
  }
  for(i=0;i<32;i++) {
      r = random(0,100);
      if(r < 80)    rc = 4;  // Most pixels will be black (off)
      else if(r<85) rc = 0;
      else if(r<90) rc = 1;
      else if(r<95) rc = 2;
      else          rc = 3;
      bufcopy[i][0] = dotcolor[rc];
      matrix.drawPixel(i,0,dotcolor[rc]);
  }

  matrix.show();

  // Gradually sequence through the color set, making a change
  // every 64 updates (so once per vertical sweep)
  clk++;
  if( (clk % 64) == 0) { changeColors(); }
...
void refill()
{
    ...
    for(i=0;i<matrix.width();i++) {
      for(j=0;j<matrix.height();j++) {
        ...
        bufcopy[i][j] = dotcolor[rc];
        matrix.drawPixel(i,j,dotcolor[rc]);
      }
    }
}
...
void changeColors() {
  ...
  j = i/4;
  dotcolor[0] = levelmask[0] & colormask[j];
  j = ((i+1)/4)%6;
  dotcolor[1] = levelmask[1] & colormask[j];
  j = ((i+2)/4)%6;
  dotcolor[2] = levelmask[2] & colormask[j];
  j = ((i+3)/4)%6;
  dotcolor[3] = levelmask[3] & colormask[j];
}

void printColors() {
  Serial.print(clk); Serial.print(" ,"); Serial.println(colclk);
  Serial.print(levelmask[0],BIN); Serial.print(", ");  Serial.println(colormask[0],BIN);
  Serial.println(dotcolor[0],BIN);
  Serial.println(dotcolor[1],BIN);
  Serial.println(dotcolor[2],BIN);
  Serial.println(dotcolor[3],BIN);
  Serial.println(dotcolor[4],BIN);
  Serial.println("------");
}
