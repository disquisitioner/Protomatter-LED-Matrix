...
// need to make sure all balls start on screen (given orientation)
int8_t  ball[5][4] = {
  {  7, 23,  1,  1 }, // Initial X,Y pos & velocity for 3 bouncy balls
  { 11, 17,  1, -1 },
  { 13, 13, -1,  1 },
  { 17, 11, -1, -1 },
  { 23,  7, -1,  1 }
...
const uint16_t ballcolor[5] = {
  0b0000000011000000, // Dark green
  0b0000000000000011, // Dark blue
  0b0001100000000000, // Dark red
  0b0001100011000000, // Dark yellow
  0b0001100000000011  // Dark purple
};
...
void setup(void) {
  ...
  for(i=0;i<5;i++) {
    ball[i][0] = random(6,(matrix.width() -6));
    ball[i][1] = random(6,(matrix.height()-6));
  }
...
void loop(void) {
  ...
  for(i=0; i<5; i++) {
    ...
    ball[i][1] += ball[i][3];
    // Bounce off edges
    if((ball[i][0] == 5) || (ball[i][0] == (matrix.width() - 6)))
      ball[i][2] *= -1;
    if((ball[i][1] == 5) || (ball[i][1] == (matrix.height() - 6)))
      ball[i][3] *= -1;
  }

  // Redraw the logo (so it's on top)
  // matrix.drawBitmap(8,4,orangemoose,48,24,0xF08800);
  draw_favicon();
  // Move the moose more slowly
  if( (count%2) == 0) {
    logox += logo_dx;
    logoy += logo_dy;
