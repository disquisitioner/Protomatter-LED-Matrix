...
// Choose the XPM image to include (this is how xpm_image gets defined)
#include "earthrise.h"
// #include "apuapu.h"
...
// Color look-up table, sized so there's room for each printable
// ASCII character to have a slot.  (And there are 95 printable ASCII
// characters, from space (#32) to ~ (#126)  (NOTE:  Shouldn't count space)
uint16_t color_lut[95];
int xpm_w,xpm_h,xpm_c,xpm_cpp;
  
void setup(void) {
  int i;

  char colindex, coltype;
  char colorval[10];
  ...
  sscanf(xpm_image[0],"%d %d %d %d",&xpm_w,&xpm_h,&xpm_c,&xpm_cpp);
  // Build color look-up table
  for(i=0;i<95;i++) { color_lut[i] = 0; }
  for(i=0;i<xpm_c;i++) {
    sscanf(xpm_image[i+1],"%c %c %s",&colindex,&coltype,colorval);
    Serial.print((int)(colindex-' '));
    Serial.print(" = ");
    Serial.println(convert_color(colorval),BIN);
    color_lut[(int)(colindex-' ')] = convert_color(colorval);
  }
  delay(20);
}
...
void loop(void) {
  ...
  xorigin = random(0,(matrix.width()-xpm_w));
  yorigin = random(0,(matrix.height()-xpm_h));
  for(j=yorigin,xj=0;j<matrix.height();j++,xj++) {
    if(xj >= xpm_h) continue;
    for(i=xorigin,xi=0;i<matrix.width();i++,xi++) {
      if(xi >= xpm_w) continue;
      cc = xpm_image[(xj+1+xpm_c)][xi];
      matrix.drawPixel(i,j,color_lut[((int)(cc-' '))]);
    }
  }
  ...
  for(i=0;i<95;i++) {
    if(color_lut[i] != 0) {
      Serial.print(i);
      Serial.print(" = ");
      Serial.println(color_lut[i],BIN);
    }
  }
  */
  delay(10000);
...
/* 
 * Convert a color definition as a #RRGGBB string 
 * into a 16 bit value for the GFX Library
*/
uint16_t convert_color(char* colstring)
{
  uint16_t colorval;
  byte r, g, b;
  
  sscanf(colstring,"#%2x%2x%2x",&r,&g,&b);
  colorval = 0;
  colorval = ((r/8) << 11) | ((g/4) << 5) | (b/8);
  // colorval = 0b1111111111111111;
  return(colorval);
}
